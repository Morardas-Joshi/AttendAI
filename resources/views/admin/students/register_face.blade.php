...
<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between mb-4">
        <h3>Register Face for {{ $student->user->name }}</h3>
                ...
                <div class="card-body text-center">
                    ...
                    @if($student->user->face_registered)
                        <div class="alert alert-info">
                            ...
                            You can capture a new photo below to overwrite their existing face data.
                        </div>
                    @endif
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        submitBtn.addEventListener("click", async () => {
            ...
            try {
                const response = await fetch("{{ route('admin.students.face.register.store', $student->id) }}", {
                    method: "POST",
                    headers: {
                ...
                if (response.ok && data.success) {
                    ...
                    toastContainer.innerHTML = `<div class="alert alert-danger mt-3">❌ ${data.message || 'Failed.'}</div>`;
